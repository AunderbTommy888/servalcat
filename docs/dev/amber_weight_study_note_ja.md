...
### 8.3 最適化器 3 版 (7dy0, 完全置き換え, 5 サイクル)
...
置き換え版では L-BFGS の優位がはっきり出る。$E_{AMBER}$ が最も低く (−9,949)、FSC も 0.9551 で
本検討を通じて 7dy0 の最高値である。目的関数が力場項中心になり、Gauss-Newton の Hessian モデル
(結合・結合角のみ) が目的関数を十分に表さなくなるため、曲率を勾配履歴から作る L-BFGS が有利になる。
L-BFGS は 5 サイクル目に nit = 0 まで落ちて収束した。非対角版は $E_{AMBER}$ と幾何がやや悪く、
置き換え時には利点が出なかった。
...
4. **L-BFGS との組み合わせが最も有望**。8.3 の結果は、置き換え + L-BFGS が FSC と $E_{AMBER}$ を
   同時に改善する唯一の組み合わせであることを示す。ただし 7dy0 のみの結果で、コストは 14 倍である。
...
- 置き換え + L-BFGS の組み合わせを 7db6 と低分解能系で確認する (8.3 は 7dy0 のみ)。
