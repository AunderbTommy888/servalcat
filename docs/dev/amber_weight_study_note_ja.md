...
## 3. 重み走査: 上乗せ版 (7db6, MT1–Gi1 + ラメルテオン, 3.3 Å)

以下 3〜6 節は**上乗せ版** ($F = w_{exp} L_{exp} + L_{geom} + w_{ff} E_{AMBER}$、現在の
`--amber_replace_geom 0`) の結果である。古典的幾何拘束を置き換える既定の挙動での結果は 8 節。
...
`--amber_replace_geom` (既定 1.0) で古典的幾何拘束を力場に置き換えると、目的関数は

$$
F = w_{exp} L_{exp} + w_{ff} E_{AMBER}
$$

になる (力場外の原子と ADP・占有率拘束は残る)。仕組みと制約は
`amber_refine_phase1_design_ja.md` の Phase 3 節。3〜6 節の実験を同じ手順でやり直した。

### 8.1 重み走査 (7db6, 5 サイクル)

![7db6 replacement weight scan](figures/fig_7db6_weight_scan_replace.png)

*図 5. 完全置き換えでの FSC と free FSC (左)、$E_{AMBER}$ (中)、辞書基準の rmsZ (右)。
$w_{ff} = 0$ は AMBER なし (古典的拘束のみ) の参照。右図の帯は AMBER と辞書の平衡値の固有の差
(|z| 0.4〜1.4) を示す。*

| $w_{ff}$ | FSC(full) | free FSC | E_AMBER 1 サイクル後 | E_AMBER 5 サイクル後 | 結合 rmsZ | 結合角 rmsZ | カイラリティ rmsZ |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 0 (AMBER なし) | 0.8294 | 0.7273 | – | – | 0.813 | 1.002 | 0.574 |
| 0.3 | 0.8014 | – | −10,130 | −32,661 | 3.023 | 1.829 | 1.337 |
| 0.5 | 0.7869 | 0.7106 | −29,123 | −52,725 | 2.156 | 1.423 | 1.020 |
| 1 (既定) | 0.7731 | 0.7032 | −37,666 | −62,278 | 1.603 | 1.178 | 0.839 |
| 2 | 0.7647 | 0.6982 | −39,827 | −64,854 | 1.426 | 1.101 | 0.787 |
| 3 | 0.7607 | 0.6967 | −40,202 | −68,583 | 1.078 | 1.028 | 0.737 |
| 5 | 0.7602 | – | −40,372 | −68,841 | 1.054 | 1.021 | 0.732 |

上乗せ版と違い、$E_{AMBER}$ はどの重みでも 5 サイクルにわたって単調に減少する。古典的拘束と
競合しなくなったためで、上乗せ版で見えた「小さい重みで $E_{AMBER}$ が上昇する」転換点は消える。
その代わり、重みの下限を決めるのは幾何の崩壊になる。$w_{ff} = 0.3$ では結合 rmsZ が 3.02 に達し、
これは辞書との平衡値の差では説明できない (帯の外)。力場が弱すぎてデータ項に押し負けている状態である。

### 8.2 クロスバリデーション

free FSC (half2) は $w_{ff} = 0$ が最大で 0.7273、置き換え版はすべてそれを下回る (0.5 で 0.7106、
1 で 0.7032、3 で 0.6967)。work − free の差は 0.064 → 0.029 (w=0.5) → 0.023 (w=1) → 0.019 (w=3) と
縮み、過学習は抑えられている。上乗せ版と同じ傾向で、この系・分解能では力場は free FSC を改善しない。

### 8.3 最適化器 3 版 (7dy0, 完全置き換え, 5 サイクル, クロスバリデーションなし)

| 版 | E_AMBER 最終 | 結合 rmsZ | 結合角 rmsZ | VDW rmsZ | FSC(full) | 実行時間 |
|---|---:|---:|---:|---:|---:|---:|
| GN + 対角 (既定) | −8,415 | 1.587 | 1.253 | 1.210 | 0.9468 | 19 s |
| GN + 非対角 + λ=100 | −7,497 | 2.007 | 1.501 | 1.174 | 0.9468 | 18 s |
| L-BFGS + 対角 | −9,949 | 1.825 | 1.625 | 1.138 | 0.9551 | 265 s |

L-BFGS が $E_{AMBER}$ を最も下げ、FSC(full) も最も高い。ただしこの表は AMBER なしの対照実験を
含まないので、この差が最適化器の効果か力場の効果か区別できない。8.4 で切り分ける。
非対角版は $E_{AMBER}$ と幾何がやや悪く、置き換え時には利点が出なかった。

### 8.4 最適化器 × 力場の 2×2 (クロスバリデーション付き)

「L-BFGS の利得は最適化器由来か、力場由来か」を切り分けるため、AMBER の有無と最適化器を
直交させて 4 条件を同一設定で走らせた (5 サイクル、AMBER 側は完全置き換え・$w_{ff} = 1$、
`--cross_validation` で half1 に対して精密化し half2 で評価)。
このために `--amber_minimizer` を `--amber_enable` なしでも使えるようにした。

```bash
.venv-openff/bin/python docs/dev/examples/compare_minimizer_x_ff.py work/minimizer_x_ff
```

**7dy0 (3.1 Å, apo)**

| 設定 | FSC(full) | work (half1) | **free (half2)** | work − free | 結合 rmsZ | 結合角 rmsZ | カイラリティ rmsZ | E_AMBER | 実行時間 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| GN, 力場なし | 0.9502 | 0.9481 | 0.9380 | 0.0101 | 0.708 | 0.902 | 0.530 | – | 17 s |
| GN, AMBER | 0.9450 | 0.9398 | 0.9359 | 0.0039 | 1.555 | 1.240 | 0.900 | −8,435 | 19 s |
| L-BFGS, 力場なし | 0.9509 | 0.9484 | 0.9392 | 0.0092 | 0.733 | 0.927 | 0.534 | – | 260 s |
| L-BFGS, AMBER | 0.9516 | 0.9490 | 0.9398 | 0.0092 | 1.656 | 1.577 | 1.120 | −10,266 | 354 s |

**7db6 (3.3 Å, + ラメルテオン)**

| 設定 | FSC(full) | work (half1) | **free (half2)** | work − free | 結合 rmsZ | 結合角 rmsZ | カイラリティ rmsZ | E_AMBER | 実行時間 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| GN, 力場なし | 0.8084 | 0.7914 | 0.7273 | 0.0641 | 0.772 | 0.944 | 0.551 | – | 65 s |
| GN, AMBER | 0.7612 | 0.7264 | 0.7032 | 0.0232 | 1.556 | 1.148 | 0.816 | −63,056 | 69 s |
| L-BFGS, 力場なし | 0.8159 | 0.7991 | 0.7320 | 0.0671 | 0.693 | 1.051 | 0.633 | – | 523 s |
| L-BFGS, AMBER | 0.8097 | 0.7911 | 0.7287 | 0.0624 | 1.660 | 1.741 | 1.314 | −77,530 | 611 s |

**L-BFGS + 対角での AMBER あり / なしの差** (この 2×2 の主眼):

| 指標 | 7dy0 なし → あり | 7db6 なし → あり |
|---|---|---|
| free FSC | 0.9392 → 0.9398 (+0.0006) | 0.7320 → 0.7287 (−0.0033) |
| FSC(full) | 0.9509 → 0.9516 (+0.0007) | 0.8159 → 0.8097 (−0.0062) |
| work − free | 0.0092 → 0.0092 (変化なし) | 0.0671 → 0.0624 (−0.0047) |
| 結合 rmsZ (辞書基準) | 0.733 → 1.656 | 0.693 → 1.660 |
| 結合角 rmsZ | 0.927 → 1.577 | 1.051 → 1.741 |
| カイラリティ rmsZ | 0.534 → 1.120 | 0.633 → 1.314 |
| 実行時間 | 260 → 354 s | 523 → 611 s |

### 8.5 議論 (2×2 から)

1. **free FSC はほぼ変わらない**。L-BFGS で AMBER を入れても free FSC は 7dy0 で +0.0006、
   7db6 で −0.0033。実験データへの当てはまりという意味では力場は中立である。
2. **L-BFGS の利得は最適化器由来**。力場なしでも GN → L-BFGS で FSC(full) が 0.9502 → 0.9509 (7dy0)、
   0.8084 → 0.8159 (7db6) と上がる。8.3 で「置き換え + L-BFGS が最良」と読めた部分は、
   対照実験を欠いた解釈であり、この 2×2 で訂正する。
3. **AMBER は GN では FSC を下げ、L-BFGS では中立**。GN で AMBER を入れると FSC(full) が
   0.9502 → 0.9450 (7dy0)、0.8084 → 0.7612 (7db6) と下がるが、L-BFGS ではほぼ変わらない。
   GN は 1 サイクル 1 ステップなので、力場項が加わって曲率モデルが目的関数と合わなくなると
   ステップが浅くなる。L-BFGS はサイクル内で反復するのでその影響を吸収する。
4. **過学習は AMBER で減るが、L-BFGS では戻る**。work − free は GN で 0.0101 → 0.0039 (7dy0)、
   0.0641 → 0.0232 (7db6) と大きく縮む。一方 L-BFGS では 0.0092 → 0.0092、0.0671 → 0.0624 と
   ほぼ変わらない。L-BFGS がデータ項をより深く最小化するため、力場による正則化の効果が相殺される。
   「力場で過学習を抑えたい」なら GN の方が目的に合う。
5. **カイラリティの劣化は L-BFGS で拡大する**。0.53 → 1.12 (7dy0)、0.63 → 1.31 (7db6) で、
   GN のとき (0.90 / 0.82) より悪い。AMBER に sp3 カイラリティ項が無いため、よく最小化するほど
   辞書のカイラリティ拘束から離れる。置き換え時は `--amber_replace_geom 0.9` などで
   古典的拘束を少し残すのが実用的である。
6. **結合・結合角 rmsZ の増加は解釈に注意**。結合の増加 (0.7 → 1.66) は AMBER と辞書の平衡値の
   固有差 (|z| 中央値 0.40、p90 1.40) でおおむね説明できる。結合角の 0.93 → 1.58 と
   カイラリティの 0.53 → 1.12 はその範囲を超えており、力場に対応物が無い/弱い項の劣化である。

### 8.6 議論 (置き換えそのもの)

1. **重みの意味が変わる**。上乗せ版の $w_{ff}$ は「拘束への追加」の強さだったが、置き換え版では
   データ項と化学の唯一の釣り合い係数になる。7db6 では 1〜3 が実用域で、既定の 1 は FSC 寄り、
   3 は幾何寄りである。$w_{ff} = 3$ の結合 rmsZ 1.078 は AMBER の最小点そのもの (帯の上端) に相当する。
2. **辞書基準の指標が使えなくなる**。置き換え後の rmsZ は「AMBER と辞書の差」を含むので、
   従来の「bond rmsZ を 0.5〜1.0 に収める」という基準は適用できない。判断軸は
   $E_{AMBER}$ の推移、free FSC、AMBER と辞書の固有差 (7dy0 で |z| 中央値 0.40、p90 1.40) との比較になる。
   自動重み調整 (`--no_weight_adjust` で無効化) も bond rmsZ を基準にしているので、
   置き換え時は $w_{exp}$ を意図せず下げる可能性がある。7db6 の $s = 0.9$ の実行では実際に 1.105 → 0.913 になった。
3. **カイラリティが唯一の実質的な劣化**。置き換えでカイラリティ rmsZ が 0.574 → 0.839 ($w_{ff} = 1$) に悪化する。
   AMBER に sp3 カイラリティ項が無いためで、$w_{ff}$ を上げても 0.73 までしか戻らない。
   `--amber_replace_geom 0.9` で 10% 残すのが実用的な保険になる。
4. **L-BFGS との組み合わせについて**: 8.3 では置き換え + L-BFGS が最良に見えたが、
   8.4 の対照実験でそれは主に最適化器の効果であり、力場は free FSC に対して中立と分かった。
   詳細は 8.5。
...
- L-BFGS を収束まで回した (maxiter を上げた) 場合に 2×2 の結論が変わるか。7db6 では 4 条件とも maxiter 20 で未収束。
- 力場が効くはずの局所指標 (リガンド周辺、側鎖末端、水の向き) で 2×2 を評価し直す。free FSC は全体指標なので力場の寄与が埋もれる。
- 置き換え時の評価軸の確立 (辞書基準の rmsZ が使えないため、free FSC と局所指標に頼る必要がある)。
...
| 置き換え版の重み走査 (7db6) | `work/7db6_weight_scan_replace/w*/`、CV は `..._replace_cv/w*/` |
| 置き換え比率の比較 (7dy0 / 7db6) | `work/replace_geom_study/` |
| 置き換え + 最適化器 3 版 (7dy0) | `work/replace_geom_study/minimizers/` |
| 最適化器 × 力場の 2×2 (7dy0 / 7db6, CV 付き) | `work/minimizer_x_ff/{dy0,db6}_{gn,lbfgs}_{noff,ff}/` |
...
| 2×2 比較スクリプト | `docs/dev/examples/compare_minimizer_x_ff.py` |
