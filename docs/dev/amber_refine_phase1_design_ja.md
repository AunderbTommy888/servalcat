...
## Phase 3: 古典的幾何拘束の置き換え

`--amber_enable` を指定したとき、既定で Servalcat の古典的幾何拘束を AMBER + OpenFF で置き換える。

$$
F = w_{exp} L_{exp} + (1 - s) \, L_{geom} + w_{ff} E_{AMBER}
$$

$s$ は `--amber_replace_geom` (既定 1.0 = 完全置き換え、0 = 従来どおり力場を上乗せ)。

### 実装

`AmberFFPrior.replace_geom_restraints(ratio)` が `RefineParams.geom_weights` (原子ごとの幾何拘束重み) を
力場が覆う原子について $(1 - s)$ 倍する。Servalcat は各拘束を「関与する原子の重みの平均」で
スケールする (`RefineParams::find_geom_weight`) ので、これだけで拘束が比率どおりに弱まる。
既存の重みに掛ける実装なので、`local_geom_weights` の設定を壊さない。

### 力場外の原子

力場に渡していない原子は重み 1 のまま残す。該当するのは次の 3 種類で、モデルには存在するが
OpenMM には渡していないため力場の勾配と Hessian がゼロになる。

1. 第 2 以降の altloc の原子 (`select_first_conformer`)。OpenMM の `PDBFile` は最初の altloc しか読まない。
2. HIE/HID 指定で外したヒスチジンのプロトン (HD1 または HE2)。
3. 同一 seqid が連続する残基 (microheterogeneity) の後続側。

力場内と力場外をまたぐ拘束は平均値になる (完全置き換えなら 0.5)。これがないと、altloc の原子が
実験項だけで動き、水素は勾配も曲率もゼロで放置される。

### AMBER に対応物が無い拘束

原子ごとの重みは全拘束種に掛かるので、次は比率どおりに弱まる。

- **カイラリティ**: AMBER の improper は sp2 の平面性のみで、sp3 のカイラリティを明示的に拘束しない。
- **NCS 拘束・スタッキング拘束**。`--ncsr` 併用時は警告する。
- **対称コピーに対する VDW 反発**: OpenMM には ASU しか渡していないので、対称・NCS 界面の衝突は見えない。

ADP・占有率・jelly body・外部拘束は原子ごとの幾何拘束重みを経由しないので影響を受けない。

### 統計の扱い

重み 0 の拘束はレポートから落ちるため、完全置き換えでは bond/angle rmsZ が消えてしまう。
`Refine.model_stats()` が統計計算のときだけ置き換え前の重みに戻すので、診断値は残り、
自動重み調整 (bond rmsZ で $w_{exp}$ を調整する機構) も働き続ける。目的関数は変わらない。

### 重み

- 力場が化学を担うので `--amber_weight` の既定は置き換え時 1.0、上乗せ時 0.1 (`resolve_amber_weight()`)。
- `--amber_weight_auto` は幾何拘束の勾配ノルムを基準にするため、置き換え時は使えない (エラーにする)。
- `--unrestrained` との併用は無意味なのでエラーにする。

### 確認結果 (7dy0, 5 サイクル, HIE, CPU)

| $s$ | $w_{ff}$ | E_AMBER 最終 | 結合 rmsZ | 結合角 rmsZ | VDW rmsZ | カイラリティ rmsZ | FSC(full) |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 0 (上乗せ) | 0.471 | −8,448 | 0.569 | 0.864 | 1.183 | 0.544 | 0.9475 |
| 0.5 | 1.0 | −8,774 | 0.972 | 1.094 | 1.199 | 0.716 | 0.9459 |
| 0.9 | 1.0 | −7,667 | 1.967 | 1.320 | 1.201 | 0.942 | 0.9470 |
| 1.0 (既定) | 1.0 | −8,415 | 1.587 | 1.253 | 1.210 | 0.909 | 0.9468 |

`geom_x` は 51,884 (置き換え前) から 0.26 に落ち、ADP 拘束の `geom_a` は残る。FSC はほぼ変わらない。

**辞書基準の rmsZ が上がる意味**: 置き換えると幾何は AMBER の平衡値に落ちるので、monomer library の
理想値を基準にした rmsZ は「悪い幾何」ではなく「AMBER と辞書の平衡値の差」を測る量になる。
7dy0 で両者を直接比較すると:

| 結合 | 本数 | 平均差 | rms 差 | &#124;z&#124; 中央値 | &#124;z&#124; p90 |
|---|---:|---:|---:|---:|---:|
| 全体 | 1,812 | −0.0030 Å | 0.0097 Å | 0.25 | 1.42 |
| 重原子間 | 913 | +0.0007 Å | 0.0086 Å | 0.40 | 1.40 |
| 水素を含む | 899 | −0.0067 Å | 0.0106 Å | 0.20 | 1.63 |

AMBER の最小点そのものが辞書基準で |z| 0.4〜1.4 に相当するので、観測された bondZ 1.0〜1.6 の主要因はこれである。
判断には E_AMBER の推移と、辞書基準の rmsZ の両方を見る必要がある。

### 確認結果 (7db6 + ラメルテオン, 5 サイクル)

置き換え時は $w_{ff}$ がデータ項との釣り合いを決める唯一の係数になる。

| 設定 | $w_{ff}$ | E_AMBER 最終 | 結合 rmsZ | 結合角 rmsZ | カイラリティ rmsZ | FSC(full) |
|---|---:|---:|---:|---:|---:|---:|
| 上乗せ ($s = 0$) | 0.259 | −60,161 | 0.590 | 0.847 | 0.553 | 0.7933 |
| 置き換え、$w_{ff} = 0.3$ | 0.3 | −32,661 | 3.023 | 1.829 | 1.337 | 0.8014 |
| 置き換え、$w_{ff} = 1$ (既定) | 1.0 | −62,278 | 1.603 | 1.178 | 0.839 | 0.7731 |
| 置き換え、$w_{ff} = 3$ | 3.0 | −68,583 | 1.078 | 1.028 | 0.737 | 0.7607 |
| 置き換え $s = 0.9$、$w_{ff} = 1$ | 1.0 | −65,023 | 1.422 | 1.148 | 0.825 | 0.7729 |

読み方:

- $w_{ff}$ を上げると E_AMBER と幾何 (辞書基準の rmsZ も含む) が改善し、FSC が下がる。
  通常のリファインにおけるデータ項と拘束項のトレードオフそのもの。
- $w_{ff} = 3$ の bondZ 1.078 は AMBER と辞書の固有の差 (|z| 0.4〜1.4) にほぼ一致する。
  つまりこの重みで構造は実質 AMBER の最小点に乗っている。
- $w_{ff} = 0.3$ では力場が弱すぎて幾何が崩れる (bondZ 3.02)。FSC は最も高いが、
  これは古典的拘束も力場もほとんど効いていない状態で、モデルとして採るべきではない。
- カイラリティ rmsZ は置き換えで 0.55 → 0.84 に悪化する。AMBER に sp3 カイラリティ項が無いため。
  カイラリティを重視する場合は `--amber_replace_geom 0.9` 程度にして 10% 残すか、
  設定ファイルで `geom_weights.chir.weight` を上げる。
- 既定の $w_{ff} = 1$ は中間的な妥協点。系ごとの再校正が必要で、辞書基準の指標が使えないため
  評価軸 (free FSC、局所指標、E_AMBER) を組み合わせる必要がある。
...
- [x] 古典的幾何拘束の置き換え (`--amber_replace_geom`、既定 1.0) と統計の維持
- [ ] 置き換え時の $w_{exp}$ と $w_{ff}$ の再校正 (辞書基準の指標が使えないので別の評価軸が必要)
